...
impl CollectionTrait for Collection{
    ...
    fn query(&mut self, query_vector: Vec<f32>) -> Result<Option<(String, f32)>, RecordError>{
        let mut index = HnswIndex::<CosineDistance>::new(self);
    ...
    fn get(&self, id: &str) -> Result<&Record, RecordError>{
        self.vectors.get(*self.id_map.get(id).unwrap()).ok_or_else(|| RecordError::RecordNotFound(id.to_string()))
    }
